...
Split integration-heavy packages into small behavioral milestones with explicit ownership, prerequisites, and acceptance evidence. The first milestone establishes the representative production boundary; each subsequent milestone adds one coherent lifecycle or consumer slice. Release dependent milestones only after the prerequisite milestone passes functional and quality review. Keep one implementation owner where state or files overlap; smaller milestones do not require more workers. After two unsuccessful corrections on the same boundary, stop repeating patches and diagnose the contract, fixture, or ownership boundary, then narrow or replan the milestone before continuing.
