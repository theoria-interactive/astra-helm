...
| `finish` | `outcome` | `summary`, `checks`, `evidence_paths`, `remaining_limitations`, `usage`, `quality_acceptance`, `delivered_work_status`, `blocker_reasons`, `assignment_dispositions`, `improvement_candidates` |
...
Telemetry correction counts measure recorded review rounds, not individual defects, all edits, tokens, or total remediation effort. A `both` round contributes to both category counters but only once to `correction_rounds`; use the distinct counter for round totals. Historical payloads without that counter have an unknown distinct total; adding category counters does not recover it. Missing historical review events remain unknown: do not backfill synthetic reviews or alter previously submitted summaries to improve the statistics.
...
Before finish, inspect each assignment’s last review. Record an evidence-backed acceptance review when corrections were actually accepted; do not manufacture acceptance to clear a warning. For a replaced or stopped package, include optional `assignment_dispositions` on finish, for example `{"original-worker": "superseded", "dropped-package": "cancelled"}`. Keys must identify dispatched assignments; values are only `superseded` or `cancelled`. Keep the replacement relationship or scope decision in local summary/evidence. A disposition explains the assignment's closure without changing its historical verdict or making an unresolved parent scope complete. If the user explicitly requests a minimized export, only the categorical disposition is included on its route; IDs and explanations stay local.

Summary routes expose `latest_review` (verdict and timestamp, or null) and `assignment_disposition`. When delivered work is accepted, summary warns about unresolved or unreviewed assignments unless explicitly superseded/cancelled. Unknown disposition IDs also warn. Inspect these as evidence gaps, not automatic proof of remaining implementation defects; preserve historical records.
...
Do not invoke or discuss sharing after finish. Only a specific user request to send activates [telemetry.md](telemetry.md). The journal remains the local evidence record; only the dedicated telemetry helper may construct and submit the minimized allowlist, after checking separate consent. Never send this journal file or its free-text fields to the receiver.
